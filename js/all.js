+ function (window, $) {
	...
	NTP.prototype.init = function () {
		...
		window.setTimeout(this.mostVisited_.checkState.bind(this), 500)
	}
	...
	MostVisited.prototype.checkState = function () {
		if (this.count < 2 && $('#most-visited-container').children().length < 2) {
			this.ui_.reset()
			this.show()
			this.count++
			window.setTimeout(this.checkState.bind(this), 1000)
		}
	}
	...
	MostVisitedUI.prototype.showUndoBar_ = function () {
		...
		obj[undo] = '#'
		obj[restoreAll] = '#'
		$('body').statusbar(chrome.i18n.getMessage('removedWebsite'), obj, {
			delay: 0,
			timerIn: 'slow',
			timerOut: 'slow'
		}, function (status, $a, e) {
